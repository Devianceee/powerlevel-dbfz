...
final class Glicko2Calculator {
  ...
  private def g(phi: Double): Double =
    1.0 / sqrt(1.0 + 3.0 * phi * phi / (Pi * Pi))

  private def expectedScore(
      mu: Double,
      opponentMu: Double,
      opponentPhi: Double
  ): Double =
  ...
  def applyDecay(
    ...
    if (periodsMissed <= 0)
      ...
      val newPhi =
        sqrt(phi * phi + periodsMissed * player.volatility * player.volatility)
  ...
  def calculate(
    ...
    if (matchResult.isEmpty)
      return RatingUpdate(player, player)
    ...
    // Calculate delta
    val delta =
      variance * matchResult.map { matchResult =>
        val expected =
          expectedScore(
            mu,
            matchResult.opponent.mu,
            matchResult.opponent.phi
          )

        g(matchResult.opponent.phi) *
          (matchResult.score - expected)
    ...
    def f(x: Double): Double = {
      ...
      val denominator =
        2.0 * math.pow(phi * phi + variance + ex, 2)
    ...
    var B =
      if (delta * delta > phi * phi + variance) {
        log(delta * delta - phi * phi - variance)
      } else {
        var k = 1
